...
def benchmark_eval(args):
    ...
    with tqdm.tqdm(total=max_iter) as pbar:
        for idx, d in enumerate(f()):
            ...
            pbar.update()
    secs = timer.seconds()
    logger.info("{} iters in {} seconds.".format(max_iter, secs))
    logger.info("FPS: {}".format(max_iter / secs))
...
if __name__ == "__main__":
    ...
    if args.adaptive_pool:
        assert args.task == "eval", "AdaptivePool is supported only in test mode"
